...
static build(scene) {
  ...
  // Pick a scale factor
  const stageScale = isMobile ? 0.4 : 0.5;
  ...
  scene.exitDoor = scene.add.image(scene.scale.width - 20, 0, "exitdoor")
    .setOrigin(0.9, 0)
    .setDepth(-10)
    .setDisplaySize(325 * (stageScale / 0.5), 128 * (stageScale / 0.5));

  // ───── Police Tape ─────
  scene.policetape = scene.add.image(scene.scale.width - 240, 65, "policetape")
    .setOrigin(0.9, 0)
    .setDepth(-10)
    .setScale(0.33 * (stageScale / 0.5));
